...
""" % (
        ...
        if params.enable_fillers:
            ...
            if len(self.regions) > 0:
                ...
                macro_width = self.node_size_x[:self.num_movable_nodes][self.movable_macro_mask]
                macro_height = self.node_size_y[:self.num_movable_nodes][self.movable_macro_mask]

            
                macro_width_with_halo = macro_width + 2 * params.macro_halo_x
                macro_height_with_halo = macro_height + 2 * params.macro_halo_y

                total_macro_halo_area = np.sum(
                    macro_width_with_halo * macro_height_with_halo - macro_width * macro_height
                )
                
                if params.use_new_filler_flag : total_filler_area_sub_macro_halo = (total_cell_space_area - total_macro_halo_area - self.total_movable_node_area) * params.target_density
                else: total_filler_area_sub_macro_halo = total_cell_space_area * params.target_density - total_movable_cell_area
                
                if(params.reduce_filler_again != 0):
                    total_filler_area_sub_macro_halo = total_filler_area_sub_macro_halo * params.reduce_filler_again
                self.total_filler_node_area = max(
                    total_filler_area_sub_macro_halo, 0.0
                )
