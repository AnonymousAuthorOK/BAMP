...
import dreamplace.ops.center_aware_macro_placement.center_aware_macro_placement as center_aware_macro_placement
...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections,
        ...
        self.iteration = 0
        self.camp_begin_iteration = params.camp_begin_iteration
        self.add_weight_per_iter = params.add_weight_per_iter
        self.center_aware_macro_weight = params.center_aware_macro_weight
        ...
        if params.center_aware_macro_placement:
            self.op_collections.center_aware_macro_placement_op = self.build_center_aware_macro_placement(placedb, self.data_collections)
    ...
    def obj_fn(self, pos):
        ...
        if self.params.center_aware_macro_placement and hasattr(self, "cur_stage"):
            if(self.cur_stage == 0) and self.iteration >= self.camp_begin_iteration:
                self.center_aware_macro_placement = self.op_collections.center_aware_macro_placement_op(pos,0)
                result = torch.add(result, self.center_aware_macro_placement, alpha=self.center_aware_macro_weight)
                self.center_aware_macro_weight += self.add_weight_per_iter
                
        if(math.isnan(result) == True):
            print("wirelength is: ",self.wirelength," density is:",self.density," center_aware_macro_placement is: ",self.center_aware_macro_placement," result is: ",result)
            print("pos_x max:", pos.max().item(), "min:", pos.min().item())
            exit()  
        return result
    ...
    def build_center_aware_macro_placement(self, placedb, data_collections):
        center_aware_macro_placement_op = center_aware_macro_placement.CenterAwareMacroPlacement(
            xl=placedb.xl,
            xh=placedb.xh,
            yl=placedb.yl,
            yh=placedb.yh,
        ...
        def build_center_aware(pos,gamma):
            return center_aware_macro_placement_op(pos,gamma)
        
        return build_center_aware
